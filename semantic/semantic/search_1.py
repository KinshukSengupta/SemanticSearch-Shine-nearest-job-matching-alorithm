...
def remove_special_chars(text):
    return re.sub(r'[^a-zA-Z\/0-9-.,+ ]','', text).lower().replace(",","/").replace("/","-").replace(" to ","-").replace("- ","-").replace(" -","-")
...
def cfg_lexical_rule(tokens,job_title,identity_skills,company,location,logical_operators):
    ...
    for j in xrange(len(tokens)):
        if tokens[j] in identity_skills and (tokens[j] not in company and tokens[j] not in location and tokens[j] not in job_title):
            tag[tokens[j]] = 'skill'
	...
	if '-' in tokens[j] and ' ' not in tokens[j]:
	    ...
	    for e in elem:
		    ...
		    tag[e] = 'job_title'
		elif e in identity_skills and tokens[j+1] in job_title:
...
if __name__ == "__main__":
    logical_operators = ['or','not','not in','no','nt in']
    query = sys.argv[1]
    try:
